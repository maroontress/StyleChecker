...
namespace TestHelper
{
    ...
    public abstract partial class DiagnosticVerifier
    {
            ...
            for (int i = 0; i < expectedResults.Length; ++i)
            {
                ...
                else
                {
                        ...
                        != expected.Locations.Length - 1)
                    {
                        Assert.IsTrue(
                            false,
                            string.Format(
                                "Expected {0} additional locations "
                                + "but got {1} for Diagnostic:\r\n"
                                + "    {2}\r\n",
                                expected.Locations.Length - 1,
                ...
                if (actual.Id != expected.Id)
                {
                    Assert.IsTrue(
                        false,
                        string.Format(
                            "Expected diagnostic id to be \"{0}\" was "
                            + "\"{1}\"\r\n"
                            + "\r\n"
                            + "Diagnostic:\r\n"
                            + "    {2}\r\n",
                ...
                if (actual.Severity != expected.Severity)
                {
                    Assert.IsTrue(
                        false,
                        string.Format(
                            "Expected diagnostic severity to be \"{0}\" was "
                            + "\"{1}\"\r\n"
                            + "\r\n"
                            + "Diagnostic:\r\n"
                            + "    {2}\r\n",
                ...
                if (actual.GetMessage() != expected.Message)
                {
                    Assert.IsTrue(
                        false,
                        string.Format(
                            "Expected diagnostic message to be \"{0}\" was "
                            + "\"{1}\"\r\n"
                            + "\r\n"
                            + "Diagnostic:\r\n"
                            + "    {2}\r\n",
            ...
            DiagnosticResultLocation expected)
        {
            ...
            Assert.IsTrue(
                actualSpan.Path == expected.Path || (actualSpan.Path != null
                && actualSpan.Path.Contains("Test0.")
                && expected.Path.Contains("Test.")),
                string.Format(
                    "Expected diagnostic to be in file \"{0}\" was "
                    + "actually in file \"{1}\"\r\n"
                    + "\r\n"
                    + "Diagnostic:\r\n"
                    + "    {2}\r\n",
            ...
            if (actualLinePosition.Line > 0)
            {
                if (actualLinePosition.Line + 1 != expected.Line)
                {
                    Assert.IsTrue(
                        false,
                        string.Format(
                            "Expected diagnostic to be on line \"{0}\" was "
                            + "actually on line \"{1}\"\r\n"
                            + "\r\n"
                            + "Diagnostic:\r\n"
                            + "    {2}\r\n",
            ...
            if (actualLinePosition.Character > 0)
            {
                if (actualLinePosition.Character + 1 != expected.Column)
                {
                    Assert.IsTrue(
                        false,
                        string.Format(
                            "Expected diagnostic to start at column \"{0}\" "
                            + "was actually at column \"{1}\"\r\n"
                            + "\r\n"
                            + "Diagnostic:\r\n"
                            + "    {2}\r\n",
                            expected.Column,
